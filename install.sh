...
*)
	...
	Remarks: only support Ubuntu 16+ / Debian 8+ / CentOS 7+ 系统
	" && exit 1
	;;
esac

# dumb detection method
...
_sys_timezone() {
	...
	echo "has set your host to Asia/Shanghai time zone and passsystemd-timesyncd Automatic time synchronization."
	echo

	if [[ $IS_OPENVZ ]]; then
		echo
		echo -e "Your host environment is ${yellow}Openvz${none} ，It is recommended to use${yellow}v2ray mkcp${none}series agreement."
		echo -e "Notice：${yellow}Openvz${none} The system time cannot be synchronized by the program control in the virtual machine."
		echo -e "If the host time is different from the actual time${yellow}more than 90 seconds${none}，v2ray will not be able to communicate normally，Please send ticket connect vps Host adjustments."
	fi
}
...
v2ray_config() {
	...
	while :; do
		...
		echo
		echo "Remark 1: Contains [dynamicPort] That is to enable dynamic ports.."
		echo "Note 2: [utp | srtp | wechat-video | dtls | wireguard] respectively disguised as [BT download| video call | WeChat video call | DTLS 1.2 data pack | WireGuard data pack]"
		echo
